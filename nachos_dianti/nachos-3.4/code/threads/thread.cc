...
{
    //allocate tid for threads
    int i;
    for (i = 0;i < 128;i++)
        if (tid_f[i] == 0){ //can be allocated
            this->tid = i;
            tid_f[i] = 1;
            tid_pointer[i] = this;
            //printf("create tid %d\n", i);
    ...
    if (i == 128){
        printf("exceed maximum thread number\n");
        return;
    }

    //priority range from 0 to 6, 0 the highest
    if (priority > 6)
        this->priority = 6;
    else if (priority < 0)
        this->priority = 0;
...
//---------------------------------------------------------------------
// Thread::threadchecknum
// check the number of thread before a thread
// to be created
//---------------------------------------------------------------------
